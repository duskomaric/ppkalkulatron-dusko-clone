...
it('tenant: user can list quotes for accessible company', function () {
    ...
    $response = $this->withHeaders(authHeaders($user))
        ->getJson("/api/v1/{$company->slug}/quotes");
...
it('tenant: user can convert quote to proforma', function () {
    $user = User::factory()->create();
    $company = Company::factory()->create();
    ...
    $response = $this->withHeaders(authHeaders($user))
        ->postJson("/api/v1/{$company->slug}/quotes/{$quote->id}/convert-to-proforma");
