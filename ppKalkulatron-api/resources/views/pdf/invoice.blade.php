...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .fiscal { margin-top: 20px; padding: 12px; background: #ecfdf5; border-radius: 6px; border-left: 4px solid #10b981; font-size: 9pt; color: #047857; }
        .fiscal a { color: #059669; }
    </style>
</head>
<body>
    <div class="page">
            ...
            <div class="header-inner">
                ...
                <div class="header-right">
                    ...
                    <div class="invoice-meta">
                        <div><strong>Datum:</strong> {{ $invoice->date->format('d.m.Y') }}</div>
                        <div><strong>Dospijeće:</strong> {{ $invoice->due_date->format('d.m.Y') }}</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="two-col">
            <div class="col">
                ...
                @if($invoice->client?->address)<div class="company-meta">{{ $invoice->client->address }}</div>@endif
                @if($invoice->client?->city || $invoice->client?->zip)
                    <div class="company-meta">{{ $invoice->client->zip }} {{ $invoice->client->city }}@if($invoice->client?->country), {{ $invoice->client->country }}@endif</div>
                @endif
                @if($invoice->client?->email)<div class="company-meta">{{ $invoice->client->email }}</div>@endif
            </div>
            <div class="col">
                @if($invoice->bankAccount)
                <div class="label">Plaćanje</div>
        ...
        <div class="totals">
            <table>
                <tr><td>Osnovica</td><td class="text-right">{{ $formatAmount($invoice->subtotal) }} {{ $currency }}</td></tr>
                <tr><td>PDV</td><td class="text-right">{{ $formatAmount($invoice->tax_total) }} {{ $currency }}</td></tr>
                <tr><td class="grand">Ukupno</td><td class="text-right grand">{{ $formatAmount($invoice->total) }} {{ $currency }}</td></tr>
            </table>
        </div>

        @if($invoice->notes)
        <div class="notes">
            <strong>Napomena:</strong><br>{{ $invoice->notes }}
        </div>
        @endif
        ...
        <div class="fiscal">
            ...
            <strong>Fiskalni broj:</strong> {{ $invoice->fiscal_invoice_number }}<br>
            @endif
            @if($invoice->fiscal_verification_url)
            <a href="{{ $invoice->fiscal_verification_url }}">Verifikacija na Poreskoj upravi</a>
            @endif
        </div>
        @endif
    </div>
