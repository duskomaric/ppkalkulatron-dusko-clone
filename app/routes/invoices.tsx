...
import {
    ...
    FileCheckIcon,
    BoxesIcon,
    FileSlidersIcon, ChevronRightIcon, ChevronLeftIcon, CalculatorIcon,
        ...
        <div className="min-h-screen bg-[#0B0B0F] flex flex-col pb-32 overflow-hidden relative">
            ...
            <header className="sticky top-0 z-40 h-[64px] flex items-center">
                <div className="max-w-[1200px] w-full mx-auto px-6 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        <div
                            className="h-10 w-10 bg-primary rounded-xl flex items-center justify-center text-white shadow-glow-purple cursor-pointer"
            ...
            <main className="flex-grow max-w-[1200px] w-full mx-auto px-6 py-8 relative z-10">
                ...
                <div className="space-y-4">
                    {invoices.map((inv) => (
                        <div key={inv.id} className="cursor-pointer bg-[#16161E]/80 backdrop-blur-xl border border-white/5 rounded-2xl transition-all duration-300 hover:bg-[#1C1C26] hover:border-primary/30 hover:shadow-[0_0_20px_rgba(168,85,247,0.15)] p-4 flex flex-col gap-2">
                            <div className="flex justify-between items-center">
                                <span className="text-sm font-bold text-white">{inv.client.name}</span>
                                <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${statusColors[inv.status_color]}`}>
                                    {inv.status_label}
                                </span>
                            </div>
                            ...
                            <div className="flex justify-between items-end">
                                <div>
                                    <p className="text-sm font-bold text-primary">{inv.invoice_number}</p>

                                    <div className="flex gap-2 text-xs font-black uppercase mt-1">
                                        <span className="text-gray-500">{inv.date}</span>
                                        <span className="text-gray-500">{inv.due_date}</span>
                                    </div>
                                </div>
            ...
            <div
              ...
              Quotes
                    ...
                    <button title="Invoices"
                            className="cursor-pointer group flex flex-col items-center gap-1 transition-all duration-300 scale-110 sm:scale-125 -translate-y-1">
                        <div
                            className="relative p-3 sm:p-3.5 rounded-2xl bg-primary/25 shadow-glow-purple ring-1 ring-primary/40 transition-all duration-300">
                            <FileTextIcon className="h-7 w-7 sm:h-8 sm:w-8 text-primary"/>
                            <span
                                className="absolute inset-0 rounded-2xl bg-primary/20 animate-pulse-slow pointer-events-none"/>
                        </div>
                        <span
            ...
            <button
                onClick={scrollToTop}
                className={`fixed right-6 bottom-32 z-[60] h-12 w-12 bg-primary rounded-full flex items-center justify-center text-white shadow-glow-purple transition-all duration-300 transform ${
            ...
            <Drawer title="Switch Company" isOpen={activeDrawer === "company"} onClose={() => setActiveDrawer(null)}>
                <div className="space-y-3">
                    {user?.companies.map((company) => (
                        ...
                        >
                            <div
                                className={`h-12 w-12 rounded-xl flex items-center justify-center font-bold text-sm shadow-glow-purple ${
            ...
            <Drawer title="My Account" isOpen={activeDrawer === 'user'} onClose={() => setActiveDrawer(null)}>
                <div className="flex items-center gap-4 p-5 bg-white/5 rounded-2xl mb-6 border border-white/5">
                    <div
                        className="h-14 w-14 bg-primary rounded-2xl flex items-center justify-center text-white font-bold text-xl shadow-glow-purple">
